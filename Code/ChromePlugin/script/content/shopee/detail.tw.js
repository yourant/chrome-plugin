...
window.onload = function() {
  /* 设置默认语言为English */
  common.setCookie('language', 'zh_TW', 1000, window.location.host.replace(/www./g, ''))
...
const detailFn = {
  ...
  async getPageDetails(product_id) {
    ...
    const attributes = await ShopeeLogic.getTWSpecifics()
  ...
  //     let str = $(imagesArray).eq(index).find('div>div').eq(0).find('div').attr('style').split(';')[0].split('(')[1]
  ...
  //     }
  //     // 去掉禁用varition选项
  //     list1.child = delDisable(list1.child)
  //     if (list2) {
  //       list2.child = delDisable(list2.child)
  //     }

  //     function delDisable(arr) {
  //       let arr1 = []
  //       Array.from(arr).forEach(v => {
  //         if ($(v).prop('className').indexOf('disabled') === -1) {
  //           arr1.push(v)
  //         }
  //       })
  //       return arr1
  ...
  //     images,
  //     product_no: url.slice(url.lastIndexOf('.') + 1, url.lastIndexOf('?')) || ''
  //   }
  //   return baseData
  // },
  // /**
  ...
  //       this.formatScript(this.formatSrc($('.page-product__content ._2C2YFD ._2aZyWI').eq(1).find('div').html().trim().replace(/(\↵|\r\n)+\s+/g, ''))) : '',
  //     packing: ''
  //   }
  //   return detail
  // },
