...
window.onload = function() {
  /* 设置默认语言为English */
  common.setCookie('language', 'tl', 1000, window.location.host.replace(/www./g, ''))
  /* 异常情况按钮不可操作 */
...
const detailFn = {
  ...
  //     }
  //     if (name !== 'Shopee' && i < $(".flex.items-center._1z1CEl.page-product__breadcrumb a").length - 1) {
  ...
  //     imagesArray.each((i, v) => {
  //       if ($(v).find('._2Fw7Qu').attr('style')) {
  //         const str = $(v).find('._2Fw7Qu').attr('style').split(';')[0].split('(')[1]
  //         let url = str.slice(str.indexOf('"') + 1, str.indexOf('_tn'))
  //         images.push(url)
  //       }
  //     })
  //     $('.shopee-icon-button._2H6_oQ').click()
  //     await common.delay(1500)
  //     index++
  //   }
  //   const arr = Array.from(new Set(images)).map(v => v.replace('_tn', ''))
  //   return arr
  // },
  ...
  //  */
  // getEvaluation() {
  //   const total_sales = $('._22sp0A').text()
  //   const total_comment = ''
  //   const good_comment = ''
  //   const comment_percent = $('.product-rating-overview__rating-score font').text().trim()
  //   return {
  //     total_sales,
  //     total_comment,
  //     good_comment,
  ...
  //   const that = this
  //   /** ====================================所有规格开始================================== **/
  //   const list1 = $('._3a2wD-').find('.flex.flex-column>div').eq(0).find('>div button:visible')
  //   const list2 = $('._3a2wD-').find('.flex.flex-column>div').eq(1).find('>div button:visible')
  //   let index = 0
  ...
  //  */
  // async getBaseData() {
  //   let product_weight, { selling_price, original_price } = await this.getPrice()
  //   const str = $('._2JMB9h').attr('style').split(';')[0].split('(')[1]
  //   let url = str.slice(str.indexOf('"') + 1, str.indexOf('_tn')).replace('"', '')
  //   let baseData = {
  //     shipping: {
  //       start_arrivals: '',
  //       delivery: $('._3Ny5of').text(),
  //       delivery_time: ''
  //     },
  //     price: {
  ...
  //       happy_price: ''
  //     },
  //     stock_num: $('._1FzU2Y .flex>div:nth-child(2)').text().replace(/[^0-9]/g, ''),
  ...
  //     if (['品牌', 'Brand', 'Merek'].includes(trElement.find('div.kIo6pj').eq(index).find('label').text().trim())) {
