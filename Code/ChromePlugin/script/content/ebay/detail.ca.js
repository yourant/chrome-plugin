...
const detailFn = {
  ...
  userInfo: {},
  init() {
    let descItemNumber = $('#descItemNumber').text() //获取ebay item number文本内容
  ...
  async checkProduct() {
    ...
    const lang = $('.gh-menu #gh-eb-Geo-a-default')
      .find('.gh-eb-Geo-txt')
      .text()
    if (lang === '简体中文') {
  ...
  async fetchProduct(product_id) {
    ...
    // 是否需要采集车型库
    if (detail.category.category_name.indexOf('Automotive') === 0) {
      detail.vehicle_accessories = ebayPureLogic.getVehicleUrls('EBAY-ENCA', detail.product_no)
    }
    console.log(detail)
    const params = {
      code: '0',
