...
const detailFn = {
  ...
  async clickIcon() {
    ...
    if (lang && lang !== 'en_US') {
      ...
      common.setCookie('lc-main', 'en_US', 365, '.amazon.fr')
      location.reload()
      return
    } else {
  ...
  //     category_name: tree_name.length ? cat.category_name.join(">") : ''
  //   };
  // },
  // /** 
  //  * @description: 获取左侧栏主图
  ...
  //       if (index === 0 && $(element).eq(index).find('a').text().trim() === 'Select' || (index === 0 && $(element).eq(index).text().trim() === 'Sélectionner')) {
  //         index++
  //         continue
  //       }
  //       /* 如果下拉框隐藏  则点击 */
  ...
  //       arr.push({
  //         key: common.formatText($(paramsList).eq(index).find('span>span').eq(0).text().trim()),
  //         value: common.formatText($(paramsList).eq(index).find('span>span').eq(1).text()).trim()
  //       })
  //       index++
  //     }
