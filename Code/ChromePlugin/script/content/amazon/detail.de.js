...
const detailFn = {
  ...
  async clickIcon() {
    ...
    if (lang && lang !== 'en_US') {
      ...
      common.setCookie('lc-main', 'en_US', 365, '.amazon.de')
      location.reload()
      return
    } else {
  ...
  async getPageDetails(product_id) {
    ...
    const product_no = AmazonLogic.getProductName()
    const month_sales = -1
    const favorite = -1
  ...
  //   // 处理三种情况 https://www.amazon.de/Olympus-Digital-Objektiv-Kameras-General%C3%BCberholt/dp/B01M3RI1Y0/ref=lp_10676133031_1_1?srs=10676133031&ie=UTF8&qid=1586400328&sr=8-1&th=1
  //   // https://www.amazon.de/Converse-Chuck-Taylor-optical-white/dp/B000OLVQ3A/ref=sr_1_2?dchild=1&pf_rd_p=81aa98d2-f61f-4dd8-b420-dd2b9961cd4c&pf_rd_r=GKZFGV2059EBQNZYNYD7&qid=1586400247&s=apparel&sr=1-2&th=1
  //   // https://www.amazon.de/LEIXNDPLBO-Japanischer-Emailtopf-Verdickung-Suppentopf/dp/B08386VHTJ/ref=sr_1_fkmr2_2?__mk_de_DE=%C3%85M%C3%85%C5%BD%C3%95%C3%91&dchild=1&keywords=Kochtopf%2Bnach%2Bjapanischer%2BArt%2Bf%C3%BCr%2BSuppentopf%2Bf%C3%BCr%2B1-2%2BPersonen%2BMilchtopf%2BEmaille%2Bohne%2BDeckel%2B-%2B1%2C5%2BLiter%2B-%2B18%2C3%2Bcm&qid=1586397671&s=kitchen&sr=1-2-fkmr2&th=1
  //   const that = this
  ...
  //     product_no: ''
  //   }
  //   const colorElement = $('#variation_style_name ul li').find('button')
  //   const colorElement1 = $('#variation_color_name ul li').find('button')
  //   if ($(colorElement).length) {
  //     let index = 0
  //     while (index < $(colorElement).length) {
  //       let perItem = {}
  //       $(colorElement).eq(index).click()
  //       await common.delay(1000)
  //       perItem.specifics = [{
  //         key: $('#variation_style_name') ? $('#variation_style_name').text().trim().split(':')[0] : '',
  //         value: $(colorElement).eq(index).text().trim()
  //       }]
  //       let innerShipping = {
  //         start_arrivals: '',
  //         delivery: '',
  ...
  //       }
  //       let price = {
  //         selling_price: $('#olp_feature_div').find('span.a-color-price') ?
  //           common.formatCurrency($('#unqualifiedBuyBox').find('span.a-color-price').eq(0).text().trim()).replace(/\s/g, '') : '0.00',
  //         original_price: '0.00',
  //         spike_price: "0.00",
  //         member_price: "0.00",
  ...
  //       }
  //       // 处理图片
  //       // const imgElement = $('#altImages>ul li[data-ux-click]')
  //       // let images = []
  //       // if(imgElement.length){
  //       //   $(imgElement).each((i, el) => {
  //       //     if ($(el).find('img').attr('src')) { 
  //       //       images.push($(el).find('img').attr('src').replace('_SX38_SY50_CR,0,0,38,50_', '_AC_UL800_'))
  //       //     }
  //       //   })
  //       // }
  //       // perItem.specifics = innerSpecifc
  //       perItem.shipping = innerShipping
  //       perItem.price = price
  //       perItem.stock_num = ''
  //       perItem.product_weight = ''
  //       perItem.packing = ''
  //       perItem.images = images
  //       perItem.product_no = ''
  //       attribute.push(perItem) // 最终数据
  //       index++
  //     }
  //   } else if ($(colorElement1).length) {
  //     let index = 0
  //     while (index < $(colorElement1).length) {
  //       let perItem = {}
  //       $(colorElement1).eq(index).click()
  //       await common.delay(1000)
  //       perItem.specifics = []
  //       if ($('#variation_size_name').length) {
  //         perItem.specifics.push({
  //           key: $('#variation_size_name') ? $('#variation_size_name').text().trim().split(':')[0].trim() : '',
  //           value: $('#variation_size_name') ? $('#variation_size_name').text().trim().split(':')[1].trim() : ''
  //         })
  //       }
  //       if ($('#variation_color_name>div.a-row').length) {
  //         perItem.specifics.push({
  //           key: $('#variation_color_name>div.a-row') ? $('#variation_color_name>div.a-row').text().trim().split(':')[0].trim() : '',
  //           value: $('#variation_color_name>div.a-row') ? $('#variation_color_name>div.a-row').text().trim().split(':')[1].trim() : ''
  //         })
  //       }
  //       let innerShipping = {
  //         start_arrivals: '',
  ...
  //       }
  //       let price = {
  //         selling_price: $('#priceblock_ourprice') ? common.formatCurrency($('#priceblock_ourprice').text().trim()).replace(/\s/g, '') : '0.00',
  //         original_price: '0.00',
  //         spike_price: '0.00',
  //         member_price: '0.00',
  //         promotion_price: '0.00',
  ...
  //     attribute.push(perItem) // 最终数据
  //   }
  //   // $(".sku-property").each((i, el) => {
  //   // 	$(el).find(".sku-property-list img").each((i, el2) => {
  //   // 		const specifics_item = {
  //   // 			key: $(el).find(".sku-title").text().trim().replace(':', ''),
  //   // 			value: ''
  //   // 		}
  //   // 		specifics_item.value = $(el2).attr("title")
  //   // 		baseData.specifics.push(specifics_item)
  //   // 		baseData.images.push($(el2).attr("src").replace(/(http:|https:)?(\/\/[a-zA-z0-9\-.]+)/g, "https:$2"))
  //   // 	})
  //   // })
  //   return attribute
  // },
  ...
  //       }
  //     })
  //     tree_name.push($(el).text().trim())
  //   })
  //   return {
  //     category_id: cat.category_id.length ? Number(cat.category_id[cat.category_id.length - 1]) : 0,
  //     category_tree: cat.category_id.length ? cat.category_id : [],
  //     category_name: tree_name.length ? tree_name.join(">") : ''
  //   };
  // },
  // /** 
  //  * @description: 获取左侧栏主图
  ...
  //       if (index === 0 && $(element).eq(index).find('a').text().trim() === 'Select' || (index === 0 && $(element).eq(index).text().trim() === 'Select')) {
  //         index++
  //         continue
  //       }
  //       /* 如果下拉框隐藏  则点击 */
  ...
  //     that = this
  //   let arr = [], outSpecific = [], shipping = {}, price = {}, product_weight, main_images = []
  ...
  //   }
  //   this.initSpecifics()
  //   detailFn.main_image = main_images
  //   return arr
  // },
  ...
  //     product_weight: product_weight || 0,
  //     images,
  //     product_no: JSON.parse($('#acBadge_feature_div script').text()).acAsin
  //   }
  //   return baseData
  // },
  // /** 
  ...
  //         const flag = await common.delayTime(2000)
  //         if (flag) {
  //           price = {
  //             selling_price: selling_price,
  //             original_price: original_price,
  ...
  //             stock_num: await that.getStockNum(),
  //             product_weight: product_weight || 0,
  //             images: [common.formatSrc($(skuList).eq(count).find('img').attr('src')).replace(/(_SX38_SY50_CR,0,0,38,50_)|(_AC_US40_)/g, '_AC_UL800_')]
  //           }
  ...
  //   newPrice = $('#olp-upd-new a').length ? common.formatCurrency($('#olp-upd-new a').text()).replace(/\s/g, '') : '0.00'
  //   selling_price = $('#price table tr').length ? common.formatCurrency($('#price table tr').eq(1).find('td').eq(1).find('span').eq(0).text().trim()).replace(/\s/g, '') : newPrice
  //   return {
  //     selling_price: selling_price || '0.00',  // 售价
  //     original_price: common.formatCurrency($('#price table tr').eq(0).find('td').eq(1).find('span').eq(0).text().trim()).replace(/\s/g, '') || '0.00' // 原价
  //   }
  // },
  // /** 
  ...
  //       let src = $(altImages).eq(index).find('img').length ? common.formatAttr(common.formatSrc($(altImages).eq(index).find('img').attr('src').trim())) : ''
  //       if (src && src.indexOf('PKmb-play-button-overlay-thumb') === -1) {
  //         arr.push(src.replace('_AC_SR38,50_', '_AC_UL800_'))
  //       }
  //     }
  //     index++
  //   }
  ...
  //     description,
  //     product_information: $('#prodDetails>div').length ? common.formatSrc($('#prodDetails>div').html().trim().replace(/(\↵|\r\n)+\s+/g, '')) : '',
  //     packing: ''
  //   }
  //   return detail
  // },
  ...
  // getDetailParams() {
  //   const arr = []
  //   // const paramsList = $('#productDescription .productDescriptionWrapper')
  //   const paramsList1 = $('#detail_bullets_id tbody .content ul li')
  //   let index = 0
  //   // if($(paramsList).length){
  //   //   // while (index < $(paramsList).length) {
  //   //   //   arr.push({
  //   //   //     key: $(paramsList).eq(index).find('th').text().trim(),
  //   //   //     value: $(paramsList).eq(index).find('td').text().trim()
  //   //   //   })
  //   //   //   index++
  //   //   // }
  //   // }else 
  //   if ($(paramsList1).length) {
  //     while (index < $(paramsList1).length - 1) {
  //       arr.push({
  //         key: $(paramsList1).eq(index).text().trim().split(':')[0],
  //         value: $(paramsList1).eq(index).text().trim().split(':')[1]
  //       })
  //       index++
  //     }
  ...
  //       productDetails = $("#prodDetails").find("table tr")

  //     } else {

  //       productDetails = $("#btfContent43_feature_div").find("table tr")
  //     }
  //     while (index < $(productDetails).length) {
  //       arr.push({
  //         key: $(productDetails).eq(index).find('td').eq(0).text().trim(),
  //         value: $(productDetails).eq(index).find('td').eq(1).text().trim(),
  //       })
  //       index++
  //     }
  //     return common.deleteEmptyProperty(arr)
  ...
  //   }
  //   // const arr = []
  //   // const paramsList = $('#detailBullets_feature_div ul li')
  //   // let index = 0
  //   // if (paramsList.length > 0) {
  //   //   while (index < $(paramsList).length) {
  //   //     arr.push({
  //   //       key: $(paramsList).eq(index).find('span>span').eq(0).text().trim(),
  //   //       value: $(paramsList).eq(index).find('span>span').eq(1).text().trim()
  //   //     })
  //   //     index++
  //   //   }
  //   //   return common.deleteEmptyProperty(arr)
  ...
  //   //   while (index < $(productDetails).length) {
  //   //     arr.push({
  //   //       key: $(productDetails).eq(index).find('th').length ?
  //   //           $(productDetails).eq(index).find('th').text().trim() :
  //   //           $(productDetails).eq(index).find('td').eq(0).text().trim(),
  //   //       value: $(productDetails).eq(index).find('th').length ?
  //   //           $(productDetails).eq(index).find('td').text().trim() :
  //   //           $(productDetails).eq(index).find('td').eq(1).text().trim(),
  //   //     })
  //   //     index++
  //   //   }
  //   //   return common.deleteEmptyProperty(arr)
