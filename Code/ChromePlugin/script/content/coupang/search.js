...
const searchFn = {
  ...
  setTabUrl() {
    ...
    if (from_id || collection_sku_id) {
      $('#headerSearchBtn').removeAttr('id')
      $('#headerSearchForm a').unbind('click').on('click', function() {
        location.href = `${location.origin}${location.pathname}?q=${encodeURI($('#headerSearchKeyword').val())}&${ from_id? 'from_id=' + from_id : '' }${ collection_sku_id? 'collection_sku_id=' + collection_sku_id : '' }`
      })
      setInterval(() => {
        $('#productList a').each((i, v) => {
          if (!$(v).attr('data-href')) {
            ...
            $(v).unbind('click').on('click', function (e) {
              ...
              const params = {
                code: 'create-tab',
                url: `${location.origin}${baseUrl}${dg}${ from_id? 'from_id=' + from_id : '' }${ collection_sku_id? 'collection_sku_id=' + collection_sku_id : '' }`
              }
